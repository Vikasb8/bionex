...
import type { User, DoctorProfile, MedicalRecord } from '../../types';
    ...
    <PageWrapper>
      <div style={{ position: 'relative', zIndex: 10, padding: '60px 32px 0', maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <h1 style={{ fontSize: 'clamp(28px, 4vw, 36px)', marginBottom: '8px', fontWeight: 800, color: '#ffffff', zIndex: 10, position: 'relative' }}>Welcome, <span style={{ color: 'var(--primary)' }}>Dr. {user?.name || 'Doctor'}</span></h1>
        </motion.div>
      </div>
      <div style={dashGrid}>
        {/* Profile Card */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card hoverable style={{ height: '100%' }}>
            ...
            {!hasProfile ? (
              <div style={{ background: 'rgba(0,0,0,0.2)', padding: '24px', borderRadius: '16px', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.2)', padding: '16px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.03)' }}>
                  <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div style={{ background: 'rgba(0,0,0,0.15)', padding: '12px 16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.02)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card hoverable style={{ height: '100%' }}>
            ...
            {foundPatient && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ background: 'linear-gradient(135deg, rgba(0,229,255,0.05), transparent)', border: '1px solid rgba(0,229,255,0.2)', borderRadius: '16px', padding: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Button size="md" onClick={() => setShowAddRecord(true)} style={{ boxShadow: '0 4px 20px -5px rgba(0,229,255,0.4)' }}>+ Add Record</Button>
                </div>
              </motion.div>
            )}
          </Card>
        ...
        {foundPatient && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ gridColumn: '1 / -1' }}>
            <Card>
              ...
              {patientRecords.length === 0 ? <p style={{ fontSize: '14px', color: 'var(--text-muted)', background: 'rgba(0,0,0,0.2)', padding: '32px', borderRadius: '12px', textAlign: 'center' }}>No historical records found for this patient.</p> : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
                  {patientRecords.map((r, i) => (
                    <motion.div key={r.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px', padding: '20px', transition: 'all 0.3s ease' }} onMouseEnter={e => e.currentTarget.style.borderColor='rgba(0,229,255,0.2)'} onMouseLeave={e => e.currentTarget.style.borderColor='rgba(255,255,255,0.05)'}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                        <span style={{ fontWeight: 700, fontSize: '16px', color: 'var(--text-heading)' }}>{r.diagnosis}</span>
                        <span style={{ fontSize: '12px', color: 'var(--primary)', background: 'rgba(0,229,255,0.1)', padding: '4px 10px', borderRadius: '8px', fontWeight: 600 }}>{new Date(r.record_date).toLocaleDateString()}</span>
                      </div>
                      ...
                      {r.file_url && <a href={r.file_url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', marginTop: '16px', fontSize: '13px', color: 'var(--primary)', background: 'rgba(0,229,255,0.08)', padding: '8px 16px', borderRadius: '8px', fontWeight: 600, textDecoration: 'none', border: '1px solid rgba(0,229,255,0.2)' }}>📎 View Attachment</a>}
                    </motion.div>
                  ))}
                </div>
              )}
      ...
      <Modal isOpen={showAddRecord} onClose={() => setShowAddRecord(false)} title="Add Medical Record">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <label
              style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '24px', border: '2px dashed var(--border-light)', borderRadius: '12px', cursor: 'pointer', transition: 'all 0.2s ease', background: 'rgba(0,0,0,0.15)' }}
            ...
            {reportFiles.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '4px' }}>
                {reportFiles.map((f, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,229,255,0.06)', padding: '8px 12px', borderRadius: '8px', fontSize: '13px' }}>
      ...
      <AnimatePresence>
        {showScanner && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} style={{ background: 'rgba(10,16,30,0.95)', border: '1px solid rgba(0,229,255,0.3)', borderRadius: '24px', padding: '40px 24px', width: '90%', maxWidth: '500px', maxHeight: '85vh', overflowY: 'auto', position: 'relative', boxShadow: '0 20px 60px -10px rgba(0,229,255,0.2)', textAlign: 'center' }}>
              <button onClick={() => setShowScanner(false)} style={{ position: 'absolute', top: '16px', right: '16px', background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', fontSize: '18px', cursor: 'pointer', width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>✕</button>
              ...
              <div id="reader" style={{ width: '100%', maxWidth: '400px', margin: '0 auto 24px', border: '2px solid rgba(0,229,255,0.3)', borderRadius: '16px', overflow: 'hidden', background: '#000' }}></div>
